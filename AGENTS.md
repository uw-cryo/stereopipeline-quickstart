...
- Concept/intro pages are authored; the banner says "in development, but ready for usage" and reader contact is the GitHub issue form it links to ([ADR-0020](architecture/0020-soften-banner-issue-form.md)). Remaining `<!-- FIGURE IDEA: ... -->` comments mark figures that still need full processing runs; don't delete one unless you're replacing it with the actual figure.
- New docs figures follow [ADR-0018](architecture/0018-docs-figure-conventions.md): conceptual diagrams are hand-authored SVGs on an opaque light surface, in a `figures/` dir next to the page; data-backed figures are PNGs from real runs, with provenance stated in the page prose.
- Link jargon to the glossary with the MyST `{term}` role at its first occurrence on a page, prose only ([ADR-0019](architecture/0019-glossary-term-hover-previews.md)). A `{term}` reference to a missing entry is a build warning — add the glossary entry first.
